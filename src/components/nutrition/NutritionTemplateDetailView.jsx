...
import { nutritionTemplates } from '@/data/nutritionData'

export function NutritionTemplateDetailView() {
  const navigate = useNavigate()
  const { id } = useParams()

  // Find the template by id
  const template = nutritionTemplates.find(t => t.id === parseInt(id))
  ...
  if (!template) {
    return (
      <div className="space-y-4">
        <button
          type="button"
          onClick={() => navigate('/nutrition/templates')}
      ...
      <Card className="p-6 sm:p-8">
        ...
        <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => navigate(`/nutrition/templates/${id}/edit`)}
        ...
        {/* Info Grid */}
        <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3 border-t border-border pt-6">
          <div>
            <p className="text-xs text-ink-muted">Calories</p>
            <p className="mt-2 text-base font-medium text-ink">{template.calories}</p>
          </div>
          <div>
            <p className="text-xs text-ink-muted">Meals</p>
            <p className="mt-2 text-base font-medium text-ink">{template.mealsCount} per day</p>
          </div>
          <div>
            <p className="text-xs text-ink-muted">Type</p>
            <p className="mt-2 text-base font-medium text-ink">{template.type}</p>
          </div>
        </div>
        ...
        <div className="mb-8 border-t border-border pt-6">
          ...
          <p className="mt-2 text-base text-ink">{template.description}</p>
        </div>

        {/* Focus */}
        <div className="border-t border-border pt-6">
          <p className="text-sm text-ink-muted">Focus</p>
          <p className="mt-2 text-base text-ink">{template.focus}</p>
        </div>
      </Card>
            ...
            <table className="w-full min-w-[500px] border-separate border-spacing-0 text-sm">
              ...
              <tbody>
                {template.meals.map((meal, i) => {
                  const last = i === template.meals.length - 1
                  return (
                    <tr key={meal.id}>
                      <Td last={last}>
